...
def summary_statistics(num_workers, data_path):
    ...
    """# Histogram
    bins = 10
    histogram_values = key_tuples.map(lambda x: x[1][0])
    bin_edges, histogram = histogram_values.histogram(bins)"""
    ...
    # Calculate median
    diff = bin_count-(n/2)
    percentile = (histogram[i]-diff)/histogram[i]
    bin_range = bin_edges[i+1]-bin_edges[i]
    median = (bin_range*percentile)+bin_edges[i]
